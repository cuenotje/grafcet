...
public class Dialogs {
    ...
    // Provided Pane is inserted in the content panel. Provided callback is
    // added to buttons' onAction handler.
	...
	if (!Double.isNaN(windowCenterX)) {
	    ...
	    // Since the dialog doesn't have a width or height till it's shown,
	    // calculate its position after it's shown
		...
		public void run() {
		    ...
		    // we don't want the top left of the dialog to shoot off the
		    // screen
		    if (x < 0)
			x = 0;
		    if (y < 0)
	...
	// According to the UI spec, the width of the main message text in the
	// upper
	...
	private Pane createCenterPanel() {
		...
		protected void layoutChildren() {
		    /*
		     * According to UI guidelines, all buttons should have the
		     * same length. This function is to define the longest
		     * button in the array of buttons and set all buttons in
		     * array to be the length of the longest button.
		     */
	...
	private Node createCenterContent() {
		...
		if (inputChoices == null || inputChoices.isEmpty()) {
		    ...
		    // ChangeListener<T> changeListener = new
		    // ChangeListener<T>() {
	...
	private List<Button> createButtons() {
		...
		if (DialogType.ERROR == dialogType && throwable != null) {
		    // we've got an error dialog, which has 'OK' and 'Details..'
		    // buttons
	...
	/*
	 * bottom panel contains icon indicating the security alert level, two
	 * bullets with most significant security warnings, link label - to view
	 * more details about security warnings.
	 */
	...
	private static class RootPane extends StackPane {
	     ...
	     *           (getScene().getWindow().isFocused()) { mask |=
	     *           PSEUDO_CLASS_ACTIVE_MASK; } return mask; }
	     * 
	...
	private final static ResourceBundle dialogsResourceBundle = ResourceBundle.getBundle("bundles.dialog-resources");

	/**
	 * Method to get an internationalized string from the deployment
	 * resource.
	 */
	...
	/**
	 * Returns an <code>ImageView</code> given an image file name or
	 * resource name
	 */
